// Template Store - Textos editáveis das landing pages
// Usa {{keyword}}, {{location}}, {{zona}}, {{address}}, {{cep}}, {{referencia}}, {{phone}}, {{whatsapp}} como variáveis
...
export interface LandingTemplate {
    ...
    // Hero
    heroSubtitle: string;
    heroBadges: string[]; // ex: ["Orçamento Grátis", "Garantia 6 Meses", "Peças Originais"]

    // Serviços (array de objetos)
    services: {
        title: string;
        description: string;
        features: string[];
    ...
    // Dados padrão de contato (usados na variável {{address}}, {{phone}}, {{cep}})
...
const TEMPLATE_KEY = 'cms-seo-template';
...
export const defaultTemplate: LandingTemplate = {
    ...
    services: [
        ...
        },
        {
            ...
            description: 'Troca de bateria de notebook em {{location}} - SP. Baterias originais e compatíveis para todas as marcas.',
...
export function getTemplate(): LandingTemplate {
    try {
...
export function saveTemplate(template: LandingTemplate): void {
...
// Replace template variables with actual page data
